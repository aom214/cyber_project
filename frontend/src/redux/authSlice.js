  ...
  async ({ username, password }, { rejectWithValue }) => {
    try {
      const response = await axios.post(
        'https://cyberbackend-9i9m.onrender.com/api/v1/user/login',
        { username, password },
...
export const verifyOtp = createAsyncThunk(
  ...
  async ({ username, otp }, { rejectWithValue }) => {
    try {
      const response = await axios.post(
        `https://cyberbackend-9i9m.onrender.com/api/v1/user/${username}/Verify_Otp`,
        { otp },
        { withCredentials: true }
      );
      return response.data;
...
export const logoutUser = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.post(
        'https://cyberbackend-9i9m.onrender.com/api/v1/user/logout',
        {},
        { withCredentials: true }
      );
      return response.data;
