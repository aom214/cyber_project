...
const AddFriendsPage = ({ onLogout }) => {
  ...
  useEffect(() => {
    const fetchNonFriends = async () => {
      try {
        const response = await axios.get("https://cyberbackend-9i9m.onrender.com/api/v1/user/get_all_non_friends", {
  ...
  const sendFriendRequest = async (userId, username) => {
    try {
      const response = await axios.get(`https://cyberbackend-9i9m.onrender.com/api/v1/user/request/${userId}`, {
        withCredentials: true,
      });
